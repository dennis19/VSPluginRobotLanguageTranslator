...
void VSPluginRobotLanguageTranslator::Options::slotDeferredInitialize()
{
   ...
   VSDForAllElements(VSPluginRobotLanguageTranslator::ExtensionRotate*, extension, project->getEnvironment()->getGuiSimState()->getDatabase())
   {
      extensionList->addItem(VSDTools::compileNameOfNode(extension->getMyModelInstance()->instanceCast<VSD::Node*>()));
   }
   VSDForAllElements(VSPluginRobotLanguageTranslator::ExtensionTranslate*, extensionTranslate, project->getEnvironment()->getGuiSimState()->getDatabase())
   {
